...
export default {
  ...
  methods: {
    // 当切换tab页时 会触发
    changeTab () {
      // 通知 所有的article-list实例 告诉他们我切换页签了  把切换的页签传过去
      // article-list 组件需要  拿到传过去的页签看看是否是自己所在的页签
      // 如果是自己所在的页签  就系要 判断一下  自己的组件是否 有滚动 如果有滚动数据  就滚动对应的位置
      // 触发一个公共事件  事件名 叫切换页签  携带参数
